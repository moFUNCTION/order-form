...
export const TotalSize = ({ register, setValue, name, errors }) => {
  const {
    ...
    <>
      ...
      <Flex alignItems="center" w="100%" gap="10" flexWrap="wrap">
        ...
        <Stack>
          ...
          {errors && (
            <>
              {errors?.x && <Text color="red.600">{errors?.x?.message}</Text>}
              {errors?.y && <Text color="red.600">{errors?.y?.message}</Text>}
            </>
          )}
        </Stack>
      </Flex>
